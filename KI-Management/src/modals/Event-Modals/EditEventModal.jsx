...
class EditEventModal extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if (!prevProps.isOpen && this.props.isOpen) {
            ...
            this.props.event_small.forEach(item => {
                if (item[0] === this.props.event[0]) {
                    selectedItems.push([smallItems[smallItems.findIndex(name => name[0] === item[1])][1], item[1], 1, item[2]]);
                }
            });
            this.props.event_large.forEach(item => {
                if (item[0] === this.props.event[0]) {
                    selectedItems.push([largeItems[largeItems.findIndex(name => name[0] === item[1])][1], item[1], 2, item[2]]);
                }
            });
            ...
            smallItems.forEach(item => {
                const alreadySelected = selectedItems.some(
                    selected => selected[1] === item[0] && selected[2] === 1
                );

                if (!alreadySelected) {
                    unselectedItems.push([item[1], item[0], 1  ]);
                }
            });

            largeItems.forEach(item => {
                const alreadySelected = selectedItems.some(
                    selected => selected[1] === item[0] && selected[2] === 2
                );

                if (!alreadySelected) {
                    unselectedItems.push([item[1], item[0], 2]);
                }
            });
            ...
            this.setState({
                ...
                eventName: this.props.event[1],
                attendees: this.props.event[2],
                start: this.props.event[3],
                end: this.props.event[4],
                date: this.props.event[5],
                rooms: this.props.rooms,
                selected_room: this.props.event[6],
                show_unavailable_rooms: false,
    ...
    checkTimeAvailable = (start, end, room, date) => {
        ...
        for (const event of this.state.events) {
            if (event[6] !== room || event[5] !== date || event[0] === this.state.event[0]) continue;
        ...
        try {
            ...
            if(this.state.selected_items.length > 0)
            {
                const response2 = await fetch("http://localhost:5000/events", {
                    ...
                    body: JSON.stringify({
                        event_id: this.state.event[0],
                        items: this.state.selected_items
                    })
                });
                const result2  = await response2.json();
                console.log("event edited:", result2);
            }
    ...
    deleteEvent = async() => {
        try {
            ...
            console.log("Delele event error:", error);
            throw error;
        }
    ...
    handleSubmit = async() => {
        ...
        else
        {
            ...
            const data = {event_id: this.state.event[0], name: eventName, attendees, start_time: start, end_time: end, date, room: selectedRoom[0]};
                ...
                <div style={styles.modal}>
                    ...
                    <div style={styles.form}>
                        <div style={styles.field}>
                            ...
                            <input name="eventName" value={this.state.eventName || ""} onChange={this.handleChange} style={inputStyle} />
                        </div>
                        <div style={styles.field}>
                            <label style = {styles.label}>Number of People Attending:</label>
                            <input type="number" name="attendees" value={this.state.attendees || ""} onChange={this.handleChange} style={inputStyle} />
                        </div>
...
const styles = {
    ...
    submit: {flex: 1, background: "#4caf50", color: "white", border: "none", padding: "6px", borderRadius: "4px",
        cursor: "pointer"},
    cancel: {flex: 1, background: "#000000", color: "white", border: "none",
